...
import { Avatar, Header, Divider, Card, ListItem, Button, Rating, PricingCard } from 'react-native-elements';
    ...
    fetch(url, {
      ...
      .then((result) => {
        ...
        for (var m = 0; m < result.length; m++) {
          result[m].image = 'data:image/png;base64,' + result[m].image;
        }
        this.setState({
          obj: result,
        });
        ...
        for (var n = 0; n < result.length; n++) {
          if (data.indexOf(result[n].proname) === -1) {
            data.push(result[n].proname);
            number++;
          }
        }
        ...
        data = [];
        for (var n = 0; n < result.length; n++) {
          if (data.indexOf(result[n].result) === -1) {

            data.push(result[n].result);
            number++;
          }
        }
    ...
    return (
          ...
          <View key="parallax-header" style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', flex: 1, }}>
            ...
            <View style={{ width: 240 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                {this.state.gender === 0 ?
                  <Image source={require('./Assets/MinePage/male.png')} style={{ marginLeft: 5 }}></Image>
                  :
      ...
      >
        ...
        {/* <ActionButton buttonColor="rgba(231,76,60,1)" buttonColor='#9b59b6' offsetX={20} offsetY={100} onPress={() => this.props.navigation.navigate('TravelNoteEdit')}>
          <ActionButton.Item buttonColor='#1abc9c' title="省份" onPress={() => this.props.navigation.navigate('ProDetails')}>
